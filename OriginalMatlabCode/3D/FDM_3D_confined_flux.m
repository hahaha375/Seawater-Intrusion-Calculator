...
K0 = load('K1.mat').K0; % Attention!!!! y,x,z, order as SEAWAT
K0 = K0(:,:,end:-1:1);
...
K = [K; K(end,:)];
% Define the IC
phi0 = (alpha*Hs-(alpha-1)*H)*ones(n, m);
zeta = 0;
xt = alpha*H*Hs-0.5*H^2; % potential at xt
...
for iter = 1:100
    ...
    for i = 1:m % i represent the i index in the difference equation along x direction
        for j =1:n % j represent the j index in the difference equation along y direction
            ...
            if i < m % if not on the right boundary
                ...
                index_i(k, 1) = k;
                index_j(k, 1) = k;
                value(k, 1) = -K(j,i)/delta_x^2-(K(j+1,i)-K(j,i))/delta_y^2-2*K(j,i)/delta_y^2;
                F(k) = Q0/delta_x-I(j, i);
            end
    ...
    phi1(filter1) = ((temp-alpha*H*Hs+0.5*alpha*H^2)*2*(alpha-1)).^0.5-(alpha-1)*H+alpha*Hs;
    phi1(filter1) = real(phi1(filter1))-imag(phi1(filter1));
    temp = Phi_star(~filter1);
    phi1(~filter1) = (temp+0.5*H^2)/H;

    % Solving interface
    zeta = alpha/(alpha-1)*Hs-1/(alpha-1)*phi1;
    zeta_temp = zeta;
    zeta(~filter1) = 0;
    zeta(zeta>H) = H; % process the pumping well(the interface could exceed the aquifer top)
...
% solution
figure()
[M1, ~] = contour(squeeze(X1(:,:,1)), squeeze(Y1(:,:,1)), Phi_star, [alpha*H*Hs-0.5*H^2 alpha*H*Hs-0.5*H^2]);
