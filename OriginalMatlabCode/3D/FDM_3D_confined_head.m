...
for iter = 1:100
    ...
    filter = inpolygon(squeeze(X1(:,:,1)),squeeze(Y1(:,:,1)),x_toe,y_toe) &...
                Phi_star<=xt; % (the potential will below the sea, leading to complex number)
    temp = Phi_star(filter);
    phi1(filter) = ((temp-alpha*H*Hs+0.5*alpha*H^2)*2*(alpha-1)).^0.5-(alpha-1)*H+alpha*Hs;
    phi1(filter) = real(phi1(filter))-imag(phi1(filter));
    temp = Phi_star(~filter);
    phi1(~filter) = (temp+0.5*H^2)/H;
    ...
    zeta(zeta<0) = 0;
    zeta(~filter1) = 0;
    zeta(zeta>H) = H; % process the pumping well(the interface will exceed the aquifer top)
    nr1 = floor(zeta);
    nr2 = ceil(zeta);
    % calculate error
