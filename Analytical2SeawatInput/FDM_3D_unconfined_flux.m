...
for iter = 1:100
    ...
    zeta_temp = zeta;
    zeta(~filter1) = 0; % process the pumping well(the interface will exceed the aquifer top)
...
fig = figure();
surf(squeeze(X1(:,:,1)), squeeze(Y1(:,:,1)), phi1, 'EdgeColor','none');light;
xlabel('\itx \rm(m)')
ylabel('\ity \rm(m)')
zlabel('\itz \rm(m)')
title('\fontname{SimSun}含水层中水位分布')
view([-5.4 8.999987545885]);
% print(fig, 'watersurface', '-dpng', '-r300')

% save the result
save('ZETA.mat', "zeta", "M1", "phi1");